<?php
	/* ------------------------------------------------
	 userUpdate.php: 
		...
		approve edit if valid,
		otherwise the user is requested to refill/retry.

	Parameters: 

		username - uneditable, the users user name
	...
	$requestUsername = $data["profile_UserName"];
	$requestPassword = $data["profile_Password"];
	$requestReenterpassword = $data["profile_reEnterPassword"];
	$requestEmail = $data["profile_email"];
	$requestDob = $data["profile_dob"];
	...
	if($GLOBALS['userobj']->updateProfile($_SESSION['dbconn'], $requestUsername, $requestPassword, $requestEmail, $requestDob) == true)
	{
		$reply['status']= 'ok';
	} 
	else 
